...
export default function SyncCenter() {
  ...
  const doSync = async () => {
    ...
    try {
      ...
      Alert.alert('Sync complete', `Sent ${result.pushed} submission(s), downloaded ${result.pulledAssignments} lesson(s).`);
    } catch (err) {
  ...
  return (
    <Screen header={<AppBar title="Sync Center" role="student" />} onRefresh={load} refreshing={checking}>
      ...
      <View style={styles.statsRow}>
        ...
        <StatCard label="Network" value={netType} icon="online" accent={colors.blue} accentSoft={colors.blueSoft} />
      </View>

      <AppCard>
        <Text style={styles.cardTitle}>Sync your work</Text>
        <Text style={styles.body}>
          Syncing sends your completed quizzes to the teacher and downloads any new lessons. You only need this when you
          are back near the hub.
        </Text>
...
const styles = StyleSheet.create({
  ...
  detail: { fontFamily: fonts.regular, fontSize: 12, color: colors.textFaint, marginTop: spacing.xs, marginBottom: spacing.md },
  statsRow: { flexDirection: 'row', gap: spacing.md, marginBottom: spacing.md },
  cardTitle: { fontFamily: fonts.bold, fontSize: 16, color: colors.text, marginBottom: spacing.sm },
  body: { fontFamily: fonts.regular, fontSize: 14, color: colors.textMuted, lineHeight: 20 },
  lastSync: { fontFamily: fonts.semibold, fontSize: 13, color: colors.text, marginTop: spacing.md },
  step: { fontFamily: fonts.regular, fontSize: 14, color: colors.text, lineHeight: 22 },
});
